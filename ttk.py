...
def ttk1():
    ...
    treeview.column("股票", width=100, anchor='center') # 表示列,不显示
    #treeview.column("IP地址", width=300, anchor='center')
    
    treeview.heading("股票", text="股票") # 显示表头
    ...
    for i in range(len(name)):
        treeview.insert('', i, values=(name[i]))
    ...
    def set_cell_value(event): # 双击进入编辑状态
        try:
            name.remove(entryedit.get(0.0, "end"))#'待命名')
        except:pass
        try:
            name.remove('待命名')
        except:pass
        ...
        def saveedit():
            ...
            #print(entryedit.get(0.0, "end"))
            name.append(num(entryedit.get(0.0, "end")))
            with open('name.json', 'w') as f:
                json.dump(name, f)
            entryedit.destroy()
            okb.destroy()

        treeview.bind('<Return>', saveedit)
        okb = ttk.Button(root, text='OK', width=3, command=saveedit)
        okb.place(x=90+(cn-1)*242,y=2+rn*20)
    
    def newrow():
        name.append('待命名')
        #ipcode.append('IP')
        treeview.insert('', len(name)-1, values=(name[len(name)-1]))
        treeview.update()
        newb.place(x=20, y=(len(name)-1)*20+45)
        newb.update()
    
    treeview.bind('<Double-1>', set_cell_value) # 双击左键进入编辑
    newb = ttk.Button(root, text='新建', width=10, command=newrow)
    newb.place(x=20,y=(len(name)-1)*20+45)
